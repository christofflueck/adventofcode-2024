        ...
        for i in range(len(text)): 
            ...
            for j in range(len(self.words)): 
                if (self.out[current_state] & (1<<j)) > 0: 
                    ...
                    result[i-len(word)+1].append(len(word)) 
...
def parse_data(data: str):
    ...
    aho_chorasick = AhoCorasick(towels) 

    return aho_chorasick, patterns


def part_a(data: str) -> int:
    aho_chorasick, patterns = parse_data(data)
    
    return sum(1 if combinations > 0 else 0 for combinations in get_combinations(aho_chorasick, patterns))

def get_combinations(aho_chorasick, patterns):
    total = []
    for pattern in patterns:
        matches = aho_chorasick.search_words(pattern)
        possibilities = [0 for _ in range(len(pattern) + 1)]
        possibilities[0] = 1
        ...
        for i in range(len(pattern)):
            ...
            for match in matches[i]:
                possibilities[i + match] += possibilities[i]
        total.append(possibilities[len(pattern)])
    return total
...
def part_b(data: str) -> int:
    ...
    return sum(get_combinations(aho_chorasick, patterns))
